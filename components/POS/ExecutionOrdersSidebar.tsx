...
export default function ExecutionOrdersSidebar({
  ...
  return (
    <>
      ...
      <div className="w-full lg:w-72 xl:w-80 lg:flex-shrink-0 flex flex-col bg-white border-r border-neutral-200 shadow-soft absolute lg:relative inset-0 lg:inset-auto z-40 lg:z-auto h-full min-w-0">
        ...
        <div className="px-4 sm:px-5 py-4 border-b border-neutral-200 bg-gradient-to-b from-neutral-50 to-white">
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-base sm:text-lg font-bold text-neutral-800">Execution orders</h2>
            <div className="flex items-center gap-2">
              {onClose && (
                <button
                  onClick={onClose}
                  className="lg:hidden p-1.5 rounded-lg hover:bg-neutral-200 text-neutral-600 hover:text-neutral-800 transition-colors"
                  title="Close sidebar"
                >
        ...
        <input
          ...
          className="w-full px-3 py-2 text-sm border border-neutral-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent text-neutral-800 placeholder-neutral-400 bg-white"
        />
      </div>

      {/* Order List Area */}
      <div className="flex-1 overflow-y-auto p-4 bg-neutral-50/50 min-h-0">
      ...
      {selectedOrder && (
        <div className="px-4 py-3 border-t border-neutral-200 bg-white space-y-3">
      ...
      <div className="px-4 py-4 border-t border-neutral-200 bg-white space-y-2">
        {onLoadOrderIntoCart && (
          <button
            onClick={() => selectedOrder && onLoadOrderIntoCart(selectedOrder)}
            disabled={!selectedOrder || loadingOrderIntoCartId === selectedOrder.id}
            className="w-full flex items-center gap-2 px-4 py-2.5 bg-primary-500 hover:bg-primary-600 disabled:bg-neutral-50 disabled:text-neutral-400 disabled:cursor-not-allowed text-white rounded-xl transition-colors font-medium text-sm"
          >
        ...
        <button
          ...
          className="w-full flex items-center gap-2 px-4 py-2.5 bg-neutral-100 hover:bg-primary-50 hover:text-primary-700 disabled:bg-neutral-50 disabled:text-neutral-400 disabled:cursor-not-allowed text-neutral-700 rounded-xl transition-colors font-medium text-sm"
        >
          {orderDetailsLoading ? (
            <svg className="w-5 h-5 animate-spin" fill="none" viewBox="0 0 24 24" aria-hidden="true">
              <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
        ...
        <button
          ...
          disabled={!selectedOrder}
          className="w-full flex items-center gap-2 px-4 py-2.5 bg-danger-50 hover:bg-danger-100 disabled:bg-neutral-50 disabled:text-neutral-400 disabled:cursor-not-allowed text-danger-700 rounded-xl transition-colors font-medium text-sm"
        >
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
